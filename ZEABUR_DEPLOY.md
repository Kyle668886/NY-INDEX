...
1. 登录后，在控制台点 **+ 新建项目**（Create Project），输入项目名称（如 `ny-index`），点 **创建**。
   - ⚠️ 首次使用会先让「创建组织」（填组织名即可），这是正常步骤。
   - ⚠️ 若提示要先验证账户（手机 / 信用卡 / 预存额度），请先完成验证，否则建不了项目。
2. 进入项目后，点 **+ 添加服务**（Add Service）。
3. 在弹出的来源里选 **GitHub**（或「Git 仓库」）。首次会跳转 GitHub 授权，授权后搜索并选 **`Kyle668886/NY-INDEX`**，分支选 `main`。
4. Zeabur 自动检测到 `Dockerfile`，构建方式选 **Docker**（一般已自动选好）。
...
在 Service 页面点 **配置** 标签（环境变量在这里设置），添加以下三项（本项目 `db.js` / `server.js` 已支持）：
...
- 想用自己域名：Service 页 **域名** 标签 → **添加自定义域名**，按提示在域名解析处加 CNAME 记录即可（需域名已备案才能在国内正常使用）。
