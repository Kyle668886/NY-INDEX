...
打开 https://zeabur.com → 点右上角 **Sign In** → 选 **GitHub** 登录（用拥有 `Kyle668886/NY-INDEX` 的那个 GitHub 账号授权）。
...
1. 登录后点左侧 **Projects → New Project**，给项目起名（如 `ny-index`）。
2. 进入项目后，点 **Deploy New Service** → 选 **Deploy from GitHub**。
3. 在仓库列表里选 **`Kyle668886/NY-INDEX`**，分支选 `main`。
4. Zeabur 会自动检测到 `Dockerfile`，构建方式选 **Docker**（一般已自动选好）。
5. 点 **Deploy**，进入构建（约 2–4 分钟，可看 Logs）。
...
在 Service 页面点 **Variables** 标签，添加以下三项（本项目 `db.js` / `server.js` 已支持）：
...
- 想用自己域名：Service 页 **Settings → Domains → 添加自定义域名**，按提示在域名解析处加 CNAME 记录即可（需域名已备案才能在国内正常使用）。
