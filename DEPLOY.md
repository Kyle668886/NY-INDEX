...
> 👉 **面向档口正式使用，推荐 Zeabur（国内可访问、中文界面）**，详细步骤见 **[ZEABUR_DEPLOY.md](./ZEABUR_DEPLOY.md)**。
> 下方 Render 章节仅适合你本人用 VPN 自测流程，档口无 VPN 打不开。
...
6. **环境变量**（Blueprint 已声明，部署时按需确认）：
   - `DATA_DIR` = `/app/data`（数据/图片统一落在此目录；免费套餐下为临时文件，重启会清空）
   - `JWT_SECRET` = 自动生成（建议部署后在控制台手动改成一个强随机串）
   - `ADMIN_PASSWORD` = 部署弹窗输入（管理员密码，强烈建议设置）
7. **持久盘**：当前 `render.yaml` 用**免费套餐（plan: free）**，不挂载持久盘，部署最简单。

> ⚠️ **套餐与持久盘的重要说明**
> - 当前默认 **免费套餐**，文件系统是临时的——每次重新部署都会丢失档口上传的图片、审核记录、管理员账号（仅档口名单 seed 会保留，因为它烧录在镜像里）。
> - 适合先用免费档跑通流程、做演示。若要让档口**长期自助上传不丢数据**，请把 `render.yaml` 的 `plan: free` 改为 `starter`（约 $7/月），并加回 `disk:` 段（挂载 `/app/data`）。`DATA_DIR` 已就绪，加盘即生效，无需改代码。
